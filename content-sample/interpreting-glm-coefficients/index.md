...
So here increasing ![](https://latex.codecogs.com/svg.latex?x) by 1 unit multiplies the mean value of Y by ![](https://latex.codecogs.com/svg.latex?exp(\beta_1)=1.25). The same thing is true for negative binomial glms as they have the same link function.
<br><br>
...
#### Logistic regression
<br>
Things become much more complicated in binomial glms. The model here is actually a model of log odds, so we need to start with an explanation of those. The odds of an event are the probability success divided by the probability of failure. So if the probability of success is ![](https://latex.codecogs.com/svg.latex?p) then the odds are:

![](https://latex.codecogs.com/svg.latex?\text{Odds}=\frac{p}{1-p})

As p increases, so do the odds. The equation for a logistic regression looks like this:

![](https://latex.codecogs.com/svg.latex?Y\sim\text{binomial}(p))

![](https://latex.codecogs.com/svg.latex?log\left(\frac{p}{1-p}\right)=\beta_0+\beta_1x)

Skipping some maths that is very similar to the above, we can obtain an interpretation for the coefficient of ![](https://latex.codecogs.com/svg.latex?x) in the model in terms of the odds. When we increase ![](https://latex.codecogs.com/svg.latex?x) by one unit the odds are multiplied by ![](https://latex.codecogs.com/svg.latex?exp(\beta_1)). Odds are not the most intuitive thing to interpret, but they do increase when p increases, so that if your coefficient ![](https://latex.codecogs.com/svg.latex?\beta_1) is positive, increasing ![](https://latex.codecogs.com/svg.latex?x) will increase your probability. 
...
summary(bin1)
```
So when we increase ![](https://latex.codecogs.com/svg.latex?x) by one unit, the odds of Y are multiplied by ![](https://latex.codecogs.com/svg.latex?exp(\beta_1)=2.11)
<br><br>
